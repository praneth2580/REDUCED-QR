...
import { dictionaryGroupEncode, dictionaryGroupDecode } from '../encoders/dictionary.group.encoders';
import { ComparisonTable } from '../components/ComparisonTable';
import { CopyButton } from '../components/CopyButton';

function DictionaryGroupDemoUI() {
  ...
  const handleEncode = () => {
    if (!input) return;
    const { encoded, dictionary } = dictionaryGroupEncode(input, groupSize);
    setEncoded(encoded);
    ...
    <div className='min-h-screen bg-gray-100 flex items-center justify-center p-4 text-black'>
      <div className='w-full max-w-4xl bg-white rounded-lg shadow-lg p-6'>
        <h1 className='text-3xl font-bold text-center text-gray-800 mb-6'>
          Dictionary Group Compression Demo
        </h1>
...
export default DictionaryGroupDemoUI;
